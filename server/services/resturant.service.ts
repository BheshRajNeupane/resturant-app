...
class ResturantService {
  async createResturant(
    { restaurantName, city, country, deliveryTime, cuisines }: ResturantDetails,
  ...
  async getRestaurantOrder(user: string) {
    try {
      ...
      const orders = await Order.find({ restaurant: resturant?._id })
        .populate("restaurant")
        .populate("user");
  ...
  async updateOrderStatus(
    orderId: string,
    { status: updateOrderStatus }: { status: OrderStatus }
  ) {
    try {
